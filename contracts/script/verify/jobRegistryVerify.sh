...
# JobRegistryImua proxy address (same on both networks)
PROXY_ADDRESS="0x6AB8aDC546153005DC3c4509Aa5d5D927B649E8B"

echo "=== Verifying JobRegistryImua on $NETWORK_NAME (Chain ID: $CHAIN_ID) ==="
...
echo "=== Verifying JobRegistryImua Implementation ==="
...
forge verify-contract \
  ...
  src/JobRegistryImua.sol:JobRegistryImua

if [ $? -eq 0 ]; then
  echo ""
  echo "✅ JobRegistryImua Implementation verified successfully on $NETWORK_NAME!"
  echo ""
...
echo "=== Verifying JobRegistryImua Proxy ==="
...
if [ $? -eq 0 ]; then
  echo ""
  echo "✅ JobRegistryImua Proxy verified successfully on $NETWORK_NAME!"
  echo ""
  echo "🔗 View on Explorer:"
